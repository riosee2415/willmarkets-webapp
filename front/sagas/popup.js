...
import {
  ...
  //
  POPUP_VIEWER_REQUEST,
  POPUP_VIEWER_SUCCESS,
  POPUP_VIEWER_FAILURE,
} from "../reducers/popup";
...
// SAGA AREA ********************************************************************************************************
// ******************************************************************************************************************
function popupViewerAPI(data) {
  return axios.get(`/api/popup/viewer`, data);
}

function* popupViewer(action) {
  try {
    const result = yield call(popupViewerAPI, action.data);

    yield put({
      type: POPUP_VIEWER_SUCCESS,
      data: result.data,
    });
  } catch (err) {
    console.error(err);
    yield put({
      type: POPUP_VIEWER_FAILURE,
      error: err.response.data,
    });
  }
}
...
function* watchPopupViewer() {
  yield takeLatest(POPUP_VIEWER_REQUEST, popupViewer);
}
